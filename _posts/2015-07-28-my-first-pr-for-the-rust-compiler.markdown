...
My fix for the issue (along with lots of help from eddyb on [irc][irc] (#rust)) is below.
...
infcx.probe(|_| {
    let fn_once_substs = Substs::new_trait(vec![infcx.next_ty_var()],  // M
                                           ...
                                           field_ty);
{% endhighlight %}
The call to `probe`, is necessary because the call to `next_ty_var` at `M` will modify the inferred context (`infctx`). The `probe` call will allow us to scope that change, and when the probe ends, the changes will be reverted.
